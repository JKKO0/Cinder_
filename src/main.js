...
function loadModel(path, scale, position, rotation) {
    gltfLoader.load(
        path,
        (gltf) => {
            ...
            object.traverse((child) => {
                if (child.isMesh) {
                    child.material = material1; // Apply custom material
                }
            });

            // Set size, position, and rotation
...
loadModel('Lots of hearts.glb', 500, [0, -2.2, 0], [0, 0, 0]);   // 10x bigger
loadModel('Rose.glb', 100, [1.8, -1, -2], [0, Math.PI / 4, 0]);  // Adjusted position and rotation
loadModel('Heart Eyes Face.glb', 75, [-2, -1, 1], [0, -Math.PI / 6, 0]); // New model
// RESIZE
